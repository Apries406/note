...
1. 在 Loader 中调用 `schema-utils` 校验配置对象：
...
> 提示：示例代码已上传到小册 [仓库](https://link.juejin.cn/?target=https%3A%2F%2Fgithub1s.com%2FTecvan-fe%2Fwebpack-book-samples%2Fblob%2Fmain%2Floader-validate%2Fpackage.json)。

之后，若用户传入不符合 Schema 描述的参数对象，会报类似下面这种错误提示：

![image.png](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/077c1aab02994c9a912f3614de6f60c0~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp?)

`schema-utils` 的校验能力很强，能够完美支撑起 Webpack 生态下非常复杂的参数校验需求，但官方文档非常语焉不详，翻阅源码后发现，它底层主要依赖于 [ajv](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fguide%2Fgetting-started.html) ，这是一个应用广泛、功能强大且性能优异的校验工具：

![image.png](https://p1-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/461958492c45459bb25cff296651d89e~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp?)

> 提示：`ajv` 在对象校验、JSON 序列化/反序列化方面的性能表现非常突出，许多知名[开源框架](https://link.juejin.cn/?target=https%3A%2F%2Fwww.npmjs.com%2Fbrowse%2Fdepended%2Fajv) 如：ESLint、fast-json-stringify、middy、swagger、tailwind 等底层都依赖于 `ajv`，值得我们学习、复用到业务项目中。

`ajv` 功能非常完备，基本上已经覆盖了“使用 JSON 描述对象约束”的所有场景，我们不可能在一篇文章里介绍所有细节，所以我下面只摘要介绍一些比较重要的能力与实例，更多信息建议参考 [官网](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fguide%2Fwhy-ajv.html)。

> `ajv` 数据描述格式基础知识：

`schema-utils` 内部使用 `ajv` 的 [JSON-Schema](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fjson-schema.html) 模式实现参数校验，而 JSON-Schema 是一种以 JSON 格式描述数据结构的 [公共规范](https://link.juejin.cn/?target=https%3A%2F%2Fjson-schema.org%2Fspecification.html)，使用时至少需要提供 `type` 参数，如：
...
- `const`：静态数值，属性值必须完全等于 `const` 定义，单独看 `const` 似乎作用不大，但配合 [$data](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fguide%2Fcombining-schemas.html%23data-reference) 指令的 [JSON-Pointer](https://link.juejin.cn/?target=https%3A%2F%2Fdatatracker.ietf.org%2Fdoc%2Frfc6901%2F) 能力，可以实现关联相等的效果，例如：
...
除上述介绍的基本类型与基础校验规则外，`ajv` 还提供了若干[复合校验指令](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fjson-schema.html%23compound-keywords)：

- [not](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fjson-schema.html%23not)：数值必须不符合该条件，例如：`{type: "number", not: {minimum: 3}}` 时，传入数值必须小于 3；
- [anyof](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fjson-schema.html%23anyof)：数值必须满足 `anyof` 条件之一，这是一个非常实用的指令，例如在 `css-loader` 中：
...
- [oneof](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fjson-schema.html%23oneof)：数值必须满足且只能满足 `oneof` 条件之一，例如：
...
- [allof](https://link.juejin.cn/?target=https%3A%2F%2Fajv.js.org%2Fjson-schema.html%23allof)：数值必须满足 `allof` 指定的所有条件，例如：
...
Webpack 支持以类似 `[path]/[name]-[hash].js` 方式设定 `output.filename` 即输出文件的命名，这一层规则通常不需要关注，但在编写类似 [webpack-contrib/file-loader](https://link.juejin.cn/?target=https%3A%2F%2Fgithub.com%2Fwebpack-contrib%2Ffile-loader) 这种自行输出产物文件的 Loader 时，需要由开发者自行处理产物路径逻辑。
...
`interpolateName` 功能稍弱于 Webpack 的 [Template String](https://link.juejin.cn/?target=https%3A%2F%2Fwebpack.js.org%2Fconfiguration%2Foutput%2F%23template-strings) 规则，仅支持如下占位符：
...
接下来，我们再结合 [vue-loader](https://link.juejin.cn/?target=https%3A%2F%2Fvue-loader.vuejs.org%2F) 源码进一步学习 Loader 开发的进阶技巧。`vue-loader` 是一个综合性很强的示例，它借助 Webpack 与组件的一系列特性巧妙地解决了：如何区分 Vue SFC 不同代码块，并复用其它 Loader 处理不同区块的内容？
...
1. 复制 `rules` 配置：代码第8行遍历 `compiler.options.module.rules` 数组，也就是用户提供的 Webpack 配置中的 `module.rules` 项，对每个rule执行 `cloneRule` 方法复制规则对象。
...
![image.png](https://p6-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/2ba22f6bdf754a00997c607117e6c881~tplv-k3u1fbpfcp-zoom-in-crop-mark:3024:0:0:0.awebp?)
