...
1. **数组扁平化，合法括号**
2. **异步任务看代码说输出，JS作用域问题，两数之和，高精度加法**
3. **判断对称二叉树**
4. http缓存策略
5. max-age是什么？值是什么？
6. cache-control字段了解吗？
7. expire听说过吗？
	1. max-age和expire的联系
8. https安全在什么地方
9. 对称加密和非对称加密分别加密的是什么内容
10. 最近在学什么前端技术？
11. React和Vue之间在响应式上实现的差异是什么？
12. React和Vue的使用上，学习阶段上最明显的差异
13. 函数式编程
14. React进行虚拟元素对比，大概的流程是什么？
15. 针对什么资源进行强缓存？
16. 浏览器事件循环机制
17. 如何判断一个任务是宏任务还是微任务
18. **最长回文子串**
19. CSS中的flex布局，一般会用到哪些应用场景？会用到哪些业务属性？
20. opaticy: 0; visibility: hidden; display: none有什么区别？如果要响应鼠标的事件的话，这三个属性设置了都可以触发吗？
21. `i = 1; let i = 2;`在哪行报错？
22. **异步输出**
23. 了解TS吗？
24. webpack plugin loader
25. 怎么优化首屏加载速度？
26. **三道大数相加并发限制的异步解调器，同时只能运行两个任务，怎么写？实现一个 Event 事件观察者的类**
27. 发布订阅模式
28. 不同设备的兼容性是怎么做的？
29. 媒体查询？
30. cookie session 区别
31. 为什么需要有sessionID
32. 没有sesssionId会出现什么？
33. 说一下localStorage, sessionStorage
34. 怎么设置一套用户的权限管理系统？根据用户权限判断用户是否能登录，以及登录后能看到的路由和同一个界面不同用户可能会看到不同的内容？在做不同用户同一页面展示不同内容时，怎么封装出来一个通用的设计模式？
35. **LRU缓存**
36. **loadsh.get()**
37. **深拷贝**
38. **岛屿数量，最大岛屿面积**
39. Formily原理
40. 常用的hooks
41. setTimeout和setInterval倒计时是精确的吗
42. 有什么更好的倒计时的实现方式？
43. 浏览器进程 线程
44. 结合线程讲事件循环
45. 网络进程和异步http线程关系
46. **函数柯里化**
47. **括号生成**
48. [字节前端商业化一二三面面经_牛客网 (nowcoder.com)](https://www.nowcoder.com/discuss/411136220913299456?sourceSSR=search)